...
component {
	...
	boolean function isAdobe(){
		...
		if ( server.keyExists( "boxlang" ) ) {
			variables.productVersion = server.boxlang.version;
			return;
		}

		// Adobe ColdFusion
		if ( server.keyExists( "coldfusion" ) ) {
			variables.productVersion = server.coldfusion.productversion;
		} else {
			variables.productVersion = "1.0.0";
		}
	}
